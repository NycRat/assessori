  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        ...
        class={cn(
          "group/wrapper flex min-h-svh w-full text-foreground has-[[data-variant=inset]]:bg-sidebar",
          local.class,
        )}
        {...others}
      >
  ...
  return (
    <Switch>
      <Match when={local.collapsible === "none"}>
        <div
          class={cn(
            "test w-(--sidebar-width) flex h-full flex-col bg-sidebar text-foreground",
            local.class,
          )}
          {...others}
        >
      ...
      <Match when={isMobile()}>
        <Sheet open={openMobile()} onOpenChange={setOpenMobile} {...others}>
          <SheetContent
            ...
            data-mobile="true"
            class="w-(--sidebar-width) bg-sidebar p-0 text-foreground [&>button]:hidden"
            style={{
      ...
      <Match when={!isMobile()}>
        ...
        >
          ...
          >
            <div
              data-sidebar="sidebar"
              class="flex size-full flex-col bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-border group-data-[variant=floating]:shadow"
            >
              {local.children}
            </div>
          </div>
...
const SidebarSeparator = <T extends ValidComponent = "hr">(
  ...
  return (
    <Separator
      ...
      class={cn("mx-2 w-auto bg-border", local.class)}
      {...others}
    />
  );
};
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-center text-sm outline-none ring-ring transition-[width,height,padding] hover:text-accent-foreground focus-visible:ring-2 active:bg-accent active:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-accent data-[active=true]:font-medium data-[active=true]:text-accent-foreground data-[state=open]:hover:bg-accent data-[state=open]:hover:text-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
const SidebarMenuButton = <T extends ValidComponent = "button">(
  ...
  const props = mergeProps(
    { isActive: false, variant: "underline", size: "default" },
    rawProps,
  );
